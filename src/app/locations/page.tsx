          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {locations.map((location) => (
                <Link
                  key={location.slug}
                  href={location.slug === "kimironko" ? `/locations/${location.slug}` : "#"}
                  className={`bg-white border-2 border-gray-200 rounded-lg p-6 hover:border-[#0d4f3a] hover:shadow-xl transition-all ${
                    location.slug !== "kimironko" ? "opacity-75" : ""
                  }`}
                >
                  ...
                  {location.slug === "kimironko" && (
                    <div className="mt-4">
                      <span className="inline-block bg-yellow-400 text-[#0d4f3a] px-4 py-2 rounded-full text-sm font-bold">
                        View Properties →
                      </span>
                    </div>
                  )}
